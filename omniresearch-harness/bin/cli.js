...
// Generated by metaharness — the `omniresearch-harness` CLI entry point.
//
// This is plain ESM JavaScript on purpose: it runs as-is via `npx omniresearch-harness`
...
// "bin" + "files" fields in package.json), so `npx omniresearch-harness` works the moment
...
const HARNESS_NAME = 'omniresearch-harness';

/** `omniresearch-harness init` — boot the kernel + host adapter and report status. */
async function init() {
...
/** `omniresearch-harness doctor` — verify the install end-to-end (kernel + host resolve). */
